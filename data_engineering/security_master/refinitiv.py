"""Ingest the equity universe from Refinitiv into the security master.
...
MAX_API_RETRIES = 3
MAX_UNIVERSE_PAGES: Optional[int] = None  # set to an int (e.g. 3) for test runs; None = full run
...
# Fields fetched via ld.get_data (keyed on RIC) to backfill identifiers that the
# EQUITY_QUOTES *search* returns blank for — notably ISIN and SEDOL.  This was
# the missing enrichment step: SEARCH_SELECT only yields blank ISIN/SEDOL, so
# build_frames() had nothing to write until these were pulled separately.
# NOTE: GICS is intentionally excluded here — those three levels are fetched by
# fetch_gics() to avoid duplicate TR.GICS* columns in the merge.
ENRICH_FIELDS = [
    "TR.ISIN",
    "TR.SEDOL",
    "TR.CUSIP",
...
# GICS hierarchy is Sector > Industry Group > Industry > Sub-Industry.
# security_master has exactly three slots (sector, industry_group, industry),
# so we fetch the matching three levels. TR.GICSIndustryGroup was missing and
# TR.GICSSubIndustry was fetched instead of TR.GICSIndustry, which mis-mapped
# the levels into the wrong columns.
GICS_FIELDS = [
    "TR.GICSSector",
...
def _coalesce(*series: pd.Series) -> pd.Series:
    """Return the first non-null value across the given series, position by position.

    Used to let the richer get_data enrichment (ISIN/SEDOL/etc.) override the
    blank values the discovery search returned, without losing search-sourced
    values where the enrichment lookup happened to miss.
    """
    out = series[0].copy()
    for s in series[1:]:
...
def _sanitize_for_db(d: dict[str, Any]) -> dict[str, Any]:
    """Sanitize a row dict before handing it to pyodbc for insertion.
...
def _api_call_with_retry(func: Callable[..., Any], *, description: str = "API call") -> Any:
    """
...
def _subdivide_and_fetch(exchange: str, prefix: str) -> pd.DataFrame:
    """Sub-divide the search space by appending characters to the prefix.
...
def fetch_refinitiv_universe() -> pd.DataFrame:
    """Fetch and concatenate the full Refinitiv universe.
...
def fetch_gics(rics: List[str]) -> pd.DataFrame:
    """Fetch GICS sector/industry classifications for the given RICs.

    Args:
        rics: List of Refinitiv RICs to enrich.

    Returns:
        DataFrame mapping each RIC to its GICS codes/names.
    """
...
def fetch_enrich(rics: List[str]) -> pd.DataFrame:
    """Fetch identifier enrichment (ISIN, SEDOL, CUSIP, VALOR, WKN, CommonCode, PermID).

    The discovery *search* returns ISIN/SEDOL as blank for the equity universe, so
    these must be pulled via ``ld.get_data`` (the same RIC-keyed endpoint that
    ``fetch_gics`` uses) and merged into the master frame later.  Mirrors the
    proven pattern in ``index_constituents/refinitiv.py``.

    Returns a DataFrame keyed on ``RIC`` (with ``Instrument`` renamed), or an
    empty frame with the expected columns if every batch failed.
    """
    batches = math.ceil(len(rics) / GICS_BATCH_SIZE)
    out = []

    log.info("Fetching identifier enrichment in %s batches", batches)

    for i in range(batches):
        batch = rics[i * GICS_BATCH_SIZE : (i + 1) * GICS_BATCH_SIZE]
        try:
            df = _api_call_with_retry(
                lambda b=batch: ld.get_data(universe=b, fields=ENRICH_FIELDS),
                description=f"Enrich batch {i+1}/{batches}",
            )
            df = df.rename(columns={"Instrument": "RIC"})
            out.append(df)
        except Exception as e:
            log.warning("Skipping enrichment batch %s due to failure: %s", i + 1, e)
...
def build_frames(
    ...
    Args:
        universe: De-duplicated Refinitiv universe frame.
        gics: GICS enrichment frame keyed by RIC.
        enrich: Optional additional identifier enrichment frame.

    Returns:
        Tuple of (security_master rows, vendor_xref rows).
    """
    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    df = universe.merge(gics, on="RIC", how="left")

    # Merge identifier enrichment (ISIN/SEDOL/CUSIP/VALOR/WKN/CommonCode/PermID)
    # fetched separately via get_data, since the discovery search returns these
    # blank. IMPORTANT: ld.get_data returns *friendly* column headers
    # (e.g. "ISIN", "SEDOL", "Common Code") rather than the TR.* field codes,
    # and omits fields that are all-null (e.g. VALOR/WKN/PermID for US equities).
    # Rename the ones we care about to prefixed names so they don't collide with
    # the search's own ISIN/SEDOL/CUSIP, and can be cleanly coalesced below.
    if enrich is not None and not enrich.empty:
        enrich_rename = {
            "ISIN": "_enr_isin",
            "SEDOL": "_enr_sedol",
            "CUSIP": "_enr_cusip",
            "Common Code": "_enr_common_code",
            "VALOR": "_enr_valor",
            "WKN": "_enr_wkn",
            "PermID": "_enr_perm_id",
        }
        enrich_renamed = enrich.rename(columns={k: v for k, v in enrich_rename.items() if k in enrich.columns})
        df = df.merge(enrich_renamed, on="RIC", how="left")

    # Safely identify which GICS column names Refinitiv actually returned.
    # Map the three fetched levels onto the three security_master slots:
    #   sector         <- GICS Sector
    #   industry_group <- GICS Industry Group
    #   industry       <- GICS Industry
    sector_col = "TR.GICSSector" if "TR.GICSSector" in df.columns else "GICS Sector Name"
    indgrp_col = "TR.GICSIndustryGroup" if "TR.GICSIndustryGroup" in df.columns else "GICS Industry Group Name"
    ind_col = "TR.GICSIndustry" if "TR.GICSIndustry" in df.columns else "GICS Industry Name"

    # Identifier columns: prefer the richer get_data enrichment, fall back to the
    # (often blank) discovery-search value.
    search_isin = _safe_column(df, "ISIN")
    search_sedol = _safe_column(df, "SEDOL")
    search_cusip = _safe_column(df, "CUSIP")

    enr_isin = _safe_column(df, "_enr_isin") if "_enr_isin" in df.columns else None
    enr_sedol = _safe_column(df, "_enr_sedol") if "_enr_sedol" in df.columns else None
    enr_cusip = _safe_column(df, "_enr_cusip") if "_enr_cusip" in df.columns else None

    isin_col = _coalesce(enr_isin, search_isin) if enr_isin is not None else search_isin
    sedol_col = _coalesce(enr_sedol, search_sedol) if enr_sedol is not None else search_sedol
    cusip_col = _coalesce(enr_cusip, search_cusip) if enr_cusip is not None else search_cusip
    ...
    master = pd.DataFrame(
        {
            ...
            "industry_group": _safe_column(df, indgrp_col),
            "industry": _safe_column(df, ind_col),
...
def resolve_security_ids(master: pd.DataFrame, session: Session, engine: sql.Engine) -> pd.DataFrame:
    ...
    # 2b. Backfill identifiers on EXISTING securities (true upsert, not insert-only).
    # A security already in security_master (matched on RIC, ISIN, CUSIP or FIGI)
    # keeps its original row, so a re-run would otherwise never land the ISIN/SEDOL
    # the enrichment just fetched. Update only the nullable identifier columns where
    # the new value is non-null and differs from what's stored.
    matched_mask = master["security_id"].notna()
    if matched_mask.any():
        existing_idx = existing.set_index("security_id")
        update_rows = []
        for _, row in master[matched_mask].iterrows():
            sid = int(row["security_id"])
            if sid not in existing_idx.index:
                continue
            cur = existing_idx.loc[sid]
            changes = {}
            for col in ("isin", "sedol", "cusip"):
                new_val = row[col]
                if new_val is None:
                    continue
                old_val = cur.get(col)
                old_val = None if (old_val is None or (isinstance(old_val, float) and pd.isna(old_val))) else old_val
                if old_val is None or str(old_val).strip() == "":
                    changes[col] = new_val
            if changes:
                changes["security_id"] = sid
                update_rows.append(changes)

        if update_rows:
            log.info("Updating ISIN/SEDOL/CUSIP on %s existing securities", len(update_rows))
            s = Session(engine)
            try:
                s.bulk_update_mappings(database.SecurityMaster, update_rows)  # type: ignore[arg-type]
                s.commit()
            except Exception as e:
                s.rollback()
                log.error("Failed to update existing securities: %s", e)
                raise
            finally:
                s.close()
...
def write_xref(master: pd.DataFrame, xref: pd.DataFrame, engine: sql.Engine) -> None:
    """Upsert the Refinitiv vendor xref rows for the ingested universe.

    Args:
        master: Security master frame (used to map RICs to ids).
...
def main() -> None:
    """Run the Refinitiv security-master ingestion pipeline end to end."""
    ...
    try:
        ...
        enrich = fetch_enrich(rics)
        master, xref = build_frames(universe, gics, enrich)
